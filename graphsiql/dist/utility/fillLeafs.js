...
exports.fillChildren = fillChildren;
...
function fillChildren(schema, editor) {
  var insertions = [];

  var docString = editor.getValue();
  ...
  var rawResults = (0, _graphqlLanguageServiceInterface.getAutocompleteSuggestions)(schema, docString, cur, token);
  var index = editor.indexFromPos(cur);
  var fieldNames = rawResults.map(function (res) {
    return res.label;
...
function defaultGetDefaultFieldNames(type) {
  ...
  Object.keys(fields).forEach(function (fieldName) {
    ...
    var fieldType = field.type.ofType || field.type;
    if ((0, _graphql.isLeafType)(fieldType)) {
      // print it commented out if it has required arguments
      var hasRequiredArgs = field.args.find(function (arg) {
        return arg.type.constructor.name === 'GraphQLNonNull';
      });
      if (hasRequiredArgs) {
